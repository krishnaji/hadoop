...
''' Reserved for Accumulo. Map: Memory => Reservation '''

reservedAccumulo = {4:1, 8:1, 16:2, 24:4, 32:6, 48:8, 64:8, 72:8, 96:16,
                   128:24, 256:32, 512:64}
...
def getReservedAccumuloMem(memory):
  if (reservedAccumulo.has_key(memory)):
    return reservedAccumulo[memory]
  if (memory <= 4):
    ret = 1
  elif (memory >= 512):
    ret = 64
...
def main():
  ...
  parser.add_option('-a', '--accumulo', default = "False",
                    help = 'True if Accumulo is installed, False otherwise')
  ...
  accumuloEnabled = ast.literal_eval(options.accumulo)
  
  log.info("Using cores=" +  str(cores) + " memory=" + str(memory) + "GB" +
            " disks=" + str(disks) + " hbase=" + str(hbaseEnabled) + " accumulo="
            + str(accumuloEnabled))
  ...
  if (hbaseEnabled):
    ...
    log.info("HBase Memory reserved: %d" % reservedHBaseMemory)
  reservedMem = reservedStackMemory + reservedHBaseMemory
  reservedAccumuloMemory = 0
  if (accumuloEnabled):
    reservedAccumuloMemory = getReservedAccumuloMem(memory)
    log.info("Accumulo Memory reserved: %d" % reservedAccumuloMemory)
  reservedMem = reservedStackMemory + reservedAccumuloMemory
